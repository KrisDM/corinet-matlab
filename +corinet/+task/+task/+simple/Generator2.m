classdef Generator2 < corinet.task.task.base.GeneratorBased
    
    %GENERATOR2 Simple generator-based task, delegating entirely to
    %PatternGenerator. The task can contain named outports and the
    %result is split to those outports.
    
    methods
        %
        % CONSTRUCTOR
        %
        function obj = Generator2(taskMaker)
    ...
    methods (Static = true)
        ...
        function obj = loadobj(s)
            obj = corinet.task.task.simple.Generator2(s);
            obj = reload(obj,s);
        end
    end
    ...
    methods 
        ...
        function context = generate(obj,context)
            ...
            for o=1:numel(obj.outportNames)
                context.(obj.outportNames{o}) = num2cell(rsult(:,o));
            end    
        end
    end
end
