function [netExecStruct,blockExecStructs,context] = ...
    meanResponse1(netExecStruct,blockExecStructs,context,blockIDs)

%MEANRESPONSE1 Network execute function for use in BlockNodePool networks
% Records the mean of the network activity for each iteration
...
for i=1:numIterations
        ...
        if t == 1
            for e=1:numBlocks
                ...
                for r=1:numel(rNames)
                    rsult{e}{r}{i} = blockExecStructs{e}.(rNames{r});
                end
            end
        else
            for e=1:numBlocks
                rNames = resultsToRecord{e};
                for r=1:numel(rNames)
                    rsult{e}{r}{i} = rsult{e}{r}{i} + blockExecStructs{e}.(rNames{r});
                end
            end
        end
    end
    for e=1:numBlocks
        rNames = resultsToRecord{e};
        for r=1:numel(rNames)
            rsult{e}{r}{i} = rsult{e}{r}{i}/context.steps;
        end
    end
end
